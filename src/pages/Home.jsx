...
const features = [
  {
    ...
    description:
      "Easily manage all your gym members with detailed profiles and tracking",
  ...
  },
  {
    ...
    title: "Workout Plans",
    description:
      "Create customized workout plans for each member based on their goals",
  ...
  },
  {
    ...
    title: "Diet Planning",
    description:
      "Design nutrition plans with macro tracking and meal scheduling",
    gradient: "from-emerald-500 to-green-600",
  },
  {
    icon: BarChart3,
    title: "Progress Analytics",
    description: "Track member progress with detailed charts and insights",
...
export default function Home() {
  ...
  /* ---------- AUTH CHECK (SAFE + SIMPLE) ---------- */
  useEffect(() => {
    const token = localStorage.getItem("access_token");

    if (token) {
      // demo user (replace with real API later)
      setUser({ role: "member" });
    } else {
      setUser(null);
    }
      ...
      <nav className="w-full border-b border-white/10 bg-black/40 backdrop-blur-xl">
        <div className="flex items-center justify-between h-[88px] px-6 sm:px-10 lg:px-16">
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-2xl font-bold tracking-wide text-violet-400">
              GymMate
            </span>
          </div>
          ...
          {!loading && (
            <Link to={createPageUrl(getDashboardUrl())}>
              <Button
                size="lg"
                className="px-8 py-3 text-lg bg-gradient-to-r from-violet-500 to-purple-600 hover:from-violet-600 hover:to-purple-700"
              >
      ...
      <section className="pt-20 pb-16 text-center px-6">
        ...
        >
          ...
          <p className="text-slate-400 text-lg max-w-xl mx-auto mb-8">
            Manage members, workouts, diets, and analytics in one powerful
            platform.
          </p>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {features.map((feature, i) => (
            <motion.div
              ...
              className="p-6 rounded-2xl bg-white/5 border border-white/10 hover:border-violet-500/40 hover:shadow-lg hover:shadow-violet-500/10 transition"
            >
              <div
